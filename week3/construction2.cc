  ...
  ostream& afficher(ostream& sortie) const
  {
	  if(contenu.size())
	  {
	    for(int i=contenu.size()-1; i>=0; i--)
	    {
		  ...
		  if(contenu[i].size())
		  {
		    for(unsigned int j=contenu[i].size()-1; j>=0; j--)
		    {
			  if(contenu[i][j].size())
			  {
	            for(unsigned int k=contenu[i][j].size()-1; k>=0; k--)
		        {
...
int main()
{
  ...
  */
  ...
  Brique WhiteB("plain", "white");
  Brique RedB("plain", "red");  
  Construction o1(2/WhiteB);
  Construction o2(3/RedB);
  o1-=o2;
  cout << o1;
  return 0;
}
